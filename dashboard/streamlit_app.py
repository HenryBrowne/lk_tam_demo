...
from brief.llm_brief import DEFAULT_MODEL as BRIEF_MODEL, BriefError, brief_for_account  # noqa: E402
...
@st.cache_data(show_spinner=False)
def cached_brief(account_id: str, _mtime: float) -> str:
    """One Claude call per account per data version; cached so tab switches are free."""
    conn = connect()
    try:
        return brief_for_account(conn, account_id)
    finally:
        conn.close()
...
def portfolio_view(rollups, scored, now, window_days):
    ...
    live_sessions = scored[scored["source"] == "live"]
    live_this_week = this_week[this_week["source"] == "live"]

    c = st.columns(5)
    c[0].metric("Accounts", len(rollups))
    c[1].metric("At-risk", len(at_risk))
    c[2].metric("Watch", len(watch))
    c[3].metric("Sessions (7d)", len(this_week), f"{len(live_this_week)} live")
    c[4].metric("Session-minutes (7d)", f"{this_week['duration_s'].fillna(0).sum() / 60:,.0f}")

    st.caption(
        f"**live** = a real LiveKit Cloud call (agent worker + SFU + real "
        f"metrics_collected / connection_quality events): "
        f"{len(live_sessions)} sessions across "
        f"{live_sessions['account_id'].nunique()} accounts. "
        f"**sim** = generated by scripts/simulate_accounts.py for portfolio breadth: "
        f"{len(scored) - len(live_sessions)} sessions."
    )

    live_by_acct = live_sessions.groupby("account_id").size().to_dict()
    rows = []
    for r in rollups:
        rows.append(
            {
                ...
                "Live": live_by_acct.get(r.account_id, 0),
...
def account_view(acc_id, rollups, scored, turns, quality, now, window_days):
    ...
    n_live = int((a_sessions["source"] == "live").sum())
    if n_live:
        st.caption(
            f"📡 {n_live} of these sessions are **real LiveKit Cloud calls** "
            f"(marked ◆ on the timeline); the rest are simulated."
        )
    ...
    with tabs[0]:
        ...
        if recent.empty:
            ...
            for v in ["green", "amber", "red"]:
                for src, sym, sz in [("sim", "circle", 9), ("live", "diamond", 13)]:
                    d = recent[(recent["verdict"] == v) & (recent["source"] == src)]
                    if d.empty:
                        continue
                    fig.add_trace(
                        go.Scatter(
                            x=d["started_at"], y=d["median_latency_ms"], mode="markers",
                            name=f"{'◆ live' if src == 'live' else ''} {v}".strip(),
                            marker=dict(color=VERDICT_COLOR[v], size=sz, symbol=sym,
                                        line=dict(width=1, color="#1b1f24" if src == "live" else "white")),
                            customdata=d[["room_sid", "duration_s", "poor_lost_share", "n_participants"]],
                            hovertemplate=(
                                f"{'REAL LiveKit call' if src == 'live' else 'simulated'}<br>"
    ...
    # --- 4. turn detail table ---
    with tabs[3]:
        cols = ["ts", "source", "room_sid", "ttft_ms", "ttfb_ms", "eou_ms", "total_latency_ms",
        ...
        st.caption("Most recent 100 turns. source=live is a real LiveKit call. "
                   "error_flag = the LLM or TTS call was cancelled / errored.")
    ...
    flag = f"brief_generated_{acc_id}"
    if st.button("✨ Generate brief", key=f"brief_btn_{acc_id}"):
        st.session_state[flag] = True
    if st.session_state.get(flag):
        try:
            with st.spinner(f"Drafting with {BRIEF_MODEL} ..."):
                text = cached_brief(acc_id, _db_mtime())
            st.markdown(text)
            st.caption(
                f"Generated by {BRIEF_MODEL} from the numbers on this page. The deterministic "
                "rules make the verdict; the model only writes the prose. Review before sending externally."
            )
        except BriefError as exc:
            st.error(str(exc))
    else:
        st.caption("A 6&ndash;8 sentence summary a TAM could put in front of an exec &mdash; one Claude call from this account's numbers.")
